...
def main():
    ...
    notebook.add(generate_password_frame, text = frame_name)
    ...
    def create_password_labels(_) -> None:
        ...
        and calls the show_password(label, text, index) function to show the passwords or the error.

        :param _: Necessary for calling the function when pressing the ENTER key.
        '''
        ...
        try:
            for password_label in password_labels:
                ...
                password = logic.generate_password(requested_length)
    ...
    done_button_image = tk.PhotoImage(file='done_button.png')

    done_btn = tk.Button(generate_password_frame, image = done_button_image, borderwidth = 0, command = lambda: create_password_labels(None))
